...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dcurrent = dh[:, t, :] + dprev_h
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dcurrent, caches[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dtotal_c = dnext_c + dnext_h * o * (1 - tanh_next_c**2)
    dprev_c = dtotal_c * f
    di = dtotal_c * g
    df = dtotal_c * prev_c
    dg = dtotal_c * i
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dcurrent_h, dprev_c, caches[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
